...
auto loclist::evaluate(expr_context *ctx) const -> expr_result {
  ...
  while (true) {
    ...
    } else if (expr_op >= (unsigned)DW_OP::breg0 &&
               expr_op < (unsigned)DW_OP::breg31) {
      result.location_type = expr_result::type::address;
      result.value = ctx->reg((unsigned)expr_op - (unsigned)DW_OP::breg0) +
                     loc_cur.sleb128();
    ...
    ranges_to_locations.insert(
        std::make_pair(std::make_pair(start_addr, end_addr), result));
  }
...
loclist::loclist(const unit *cu, section_offset offset)
    : cu(cu), offset(offset) {}
